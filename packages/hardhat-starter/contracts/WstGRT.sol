  ...
  MulticallUpgradeable
{
   ...
   */
  function delegate(ActioinParam[] calldata actions) external whenNotPaused onlyOperator {
    ...
    while (_index < size) {
      ...
      //update info
      if (is_new) {
        // init the delegator lastGRTPerShare
      ...
      delegatorInfo.shares = _delegation.shares;
   ...
   */
  function collectFee() public whenNotPaused {
    GStakeStorage storage $ = _getGStakeStorage();
    uint32 _percentage = getDelegationTaxPercentage();
   ...
   */
  function updateRewardAllIndexer() public whenNotPaused {
    GStakeStorage storage $ = _getGStakeStorage();
   ...
   */
  function updateReward(uint256[] memory _delegatorIndexs) public whenNotPaused {
   ...
   */
  function undelegate(uint256[] memory ids) external whenNotPaused onlyOperator {
    GStakeStorage storage $ = _getGStakeStorage();
   ...
   */
  function skipUndelegator(uint256 index) external whenNotPaused onlyOperator {
    GStakeStorage storage $ = _getGStakeStorage();
   ...
   */
  function claimUndelegation(uint256 undelegationId) external whenNotPaused {
    GStakeStorage storage $ = _getGStakeStorage();
   ...
   * @dev The coin will be transferred to the NFT's owner.
   ...
   * @param ids The NFT tokenIds of the withdraw requests.
   */
  function matchOrder(uint256[] memory ids) public whenNotPaused {
    GStakeStorage storage $ = _getGStakeStorage();
   ...
   */
  function createDelegator(uint256 num) public whenNotPaused onlyOperator {
    GStakeStorage storage $ = _getGStakeStorage();
    address _theGraphStaking = $.theGraphStaking;
    uint256 i = 0;
    while (i < num) {
  ...
  function setUndelegateInterval(uint40 period_) external onlyOwner {
    ...
    // update lastUndelegateNum
    $.lastUndelegateNum = uint40(block.timestamp / period_);
  ...
  function getUndelegateInfo(uint256 _undelegationId) public view returns (UndelegateInfo memory, address) {
    GStakeStorage storage $ = _getGStakeStorage();
    return ($.undelegations[_undelegationId], $.undelegationIndexer[_undelegationId]);
  }
  ...
  function _withdrawDelegated(
    address _theGraphStaking,
    address delegator,
    address indexer
  ) private returns (uint256 amountOfGRT) {
  ...
  function _enqueue(uint256 _amountOfWstGRT, uint256 amountOfGRT, address _owner) private returns (uint256 tokenId) {
    ...
    $.withdrawRequests[tokenId] = WithdrawalRequest(_amountOfWstGRT, amountOfGRT, 0, WRStatus.Processing, _timestamp);
  ...
  function _updateReward(
    ...
    IGraphStaking.DelegationPoolReturn memory _pool = IGraphStaking(_theGraphStaking).delegationPools(
      delegatorInfo.indexer
    );
